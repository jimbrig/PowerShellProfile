...
foreach ($prof in $firefoxProfiles) {
    Remove-Item "$($prof.FullName)\cache2\*" -Recurse -Force -ErrorAction SilentlyContinue
    Remove-Item "$($prof.FullName)\jumpListCache\*" -Recurse -Force -ErrorAction SilentlyContinue
}
Pause-Step
